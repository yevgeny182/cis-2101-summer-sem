...
int main() {
    ...
    printf("\n------------");
    closeDic * close;	
    close = convertToCloseDict(&D);
 	 displayCloseDict(*close);
    return 0;
}
...
openDic initOpenDict(VHeap *VH) {
  	...
  	int a;
  	newOpenDic.count = 0;
  	newOpenDic.dicVHptr = VH;
  	for(a=0; a<OPEN_DSIZE; a++){
  		newOpenDic.header[a] = -1;
	  }
	  return newOpenDic;
}
...
openDic convertToOpenDict(cursorSet *A) {
  int a;
  openDic newOD = initOpenDict(A->VHptr);
  	for(a=0; a<VH_SIZE; a++){
  		int hash = openHash(A->VHptr->VH_node[a].elem.prodID);
  		int *trav;
  		for(trav = &newOD.header[hash]; *trav!=-1 && strcmp(A->VHptr->VH_node[a].elem.prodID, A->VHptr->VH_node[*trav].elem.prodID) > 0; trav = &newOD.dicVHptr->VH_node[*trav].next){}
 	...
 	for(i=0; i<OPEN_DSIZE; i++){
 		printf("\nGroup:[%d]", i);
 		 for(j = D.header[i]; j!=-1; j = D.dicVHptr->VH_node[j].next){
 		 	printf("\t%-15s", D.dicVHptr->VH_node[j].elem.prodID);
		  }
	 }
...
closeDic *convertToCloseDict(openDic *D) {
   List newList;
   closeDic * Nnode = (closeDic*)malloc(sizeof(closeDic));
   newList.last = - 1;
   int a, b, hash;
   if(Nnode!=NULL){
   	initCloseDict(*Nnode);
   	for(a=0; a<CLOSE_DSIZE; a++){
   		...
   		for(trav = &D->header[a]; *trav!=-1;){
   			hash =  closeHash(D->dicVHptr->VH_node[*trav].elem.prodID);
   			int temp = *trav;
   			if(strcmp((*Nnode)[hash].prodID, EMPTY)==0){
   				(*Nnode)[hash] = D->dicVHptr->VH_node[*trav].elem;
			   }
			   else{
	   ...
	   for(a=0; a<=newList.last; a++){
	   	hash = closeHash(D->dicVHptr->VH_node[a].elem.prodID);
	for(b=0; b<CLOSE_DSIZE && strcmp((*Nnode)[hash].prodID, EMPTY)!=0 && strcmp((*Nnode)[hash].prodID, DELETED)!=0; hash = (hash + 1) %CLOSE_DSIZE, b++){}
		if(b < CLOSE_DSIZE){
			(*Nnode)[hash] = newList.prod[a];
		}
	   }
   }
   return Nnode;
...
void displayCloseDict(closeDic CD) {
  ...
  printf("\n\nDetails of Closed Hash Dictionary :: ");
	...
	printf("\n%-6s%-10s%-15s", "-----", "-------", "----------"); 
	for(a = 0; a<CLOSE_DSIZE; a++){
		printf("\n%-6d%-10s%-15s", a, CD[a].prodID, CD[a].prodDesc.name); 
	}
    printf("\n\n");
}
